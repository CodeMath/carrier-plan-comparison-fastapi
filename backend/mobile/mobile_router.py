...
@router.post("/create/mobile", response_model=model_mobile.Mobile)
async def mobile_create(
    plan: mobile_schemas.CreateMobile_Schema, db: Session = Depends(get_db)
):
    """Create Mobile Plan"""

    exist_mobile_plan = mobile_crud.get_existing_mobile(db, plan=plan)

    if exist_mobile_plan:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Already registered"
        )
    return mobile_crud.create_mobile(db=db, plan=plan)
...
@router.get(
    ...
    resonse_model=mobile_schemas.MobileList,
)
async def mobile_item_by_title(
    ...
    for plan in title_list:
        ...
        if find_plan:
            result.append(find_plan)
    return result


@router.update(
    "/update/mobile", summary="update mobile", status_cdoe=status.HTTP_204_NO_CONTENT
)
async def mobile_update(db: Session, _mobile_update=mobile_schemas.UpdateMobile_Schema):
...
@router.delete("/delete/mobile", status_code=status.HTTP_204_NO_CONTENT)
async def mobile_delete(
    db: Session, _mobile_delete: mobile_schemas.DeleteMobile_Schema
):
